...
- **权限模式**：`default` / `auto` 等授权模式，控制工具执行前是否需用户确认。权限确认双层超时：5 分钟未确认仅提示（可在弹窗中重新确认，不重发消息）；再过 25 分钟仍未响应则自动拒绝。默认在拒绝后**暂停对话**（等你点击「恢复」继续，agent 不会在无人确认时继续自主执行）；`permission.timeoutAction: 'deny'` 可改回旧行为（拒绝后对话继续，会话永不挂起）。
- **会话可恢复**：会话快照序列化/反序列化，支持进程重启后无缝恢复。
- **无感知热更新**：后台定期检查新版本，安装成功后才暂停活跃会话 → 序列化会话快照 → npm 自更新 → 端口 handoff 接管。空闲会话零感知；进行中的对话会中断（界面提示后可一键重发继续），中断前的半截回复与工具结果会在时间线置灰标记为「未完成轮次」并**从上下文剔除**（重发后不影响新对话），安装失败时不触碰任何会话状态。更新完成后请**刷新页面**加载新版界面（后续请求已自动落到新实例，不刷新则页面仍运行旧版前端）。
...
- **会话用量**：会话列表每行显示累计 token 徽标；会话页「会话信息」面板提供 token/成本/消息数等完整统计（token 为服务端上报用量，成本按配置价目估算）。设置页「用量与成本」面板提供**全局聚合**：总量、按月、按模型统计，并可设置月度成本预算（`usage.monthlyBudgetUsd`）——当月成本超预算时醒目告警。聚合涵盖**全部会话（含回收站内已删除会话）**——成本是账本，删除会话不会抹去已发生花费；价格未知的调用（自建网关/未登记模型）按 $0 计入并单独计数提示，内置价目标注最近核价日期（估算可能随价格变动过期，实际费用以账单为准）。顶栏显示**本月成本徽标**：达预算 80% 变警示色、超预算变红色，点击直达用量面板。
- **主题**：亮/暗/跟随系统主题。
